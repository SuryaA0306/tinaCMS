---
title: Theming Angular Material 2
description: Theming Angular Material 2
heroImage: /Theming angular material .png
---
...
This guide refers to Material 2, the previous version of Material. For the latest Material 3 documentation, see the [theming guide](https://material.angular.io/guide/theming).

For information on how to update, see the section on [how to migrate an app from Material 2 to Material 3](https://material.angular.io/guide/material-2-theming#how-to-migrate-an-app-from-material-2-to-material-3).

This document describes the concepts and APIs for customizing colors. For typography customization, see [Angular Material Typography](https://material.angular.io/guide/typography). For guidance on building components to be customizable with this system, see [Theming your own components](https://material.angular.io/guide/theming-your-components).
...
You can use Angular Material without Sass by using a pre-built theme, described in [Using a pre-built theme](https://material.angular.io/guide/material-2-theming#using-a-pre-built-theme) below. However, using the library's Sass API directly gives you the most control over the styles in your application.
...
You can create your own palette by defining a Sass map that matches the structure described in the [Palettes](https://material.angular.io/guide/material-2-theming#palettes) section above. The map must define hues for 50 and each hundred between 100 and 900. The map must also define a contrast map with contrast colors for each hue.
...
In addition to hues numbered from zero to 900, the 2014 Material Design palettes each include distinct accent hues numbered as A100, A200, A400, and A700. Angular Material does not require these hues, but you can use these hues when defining a theme as described in [Defining a theme](https://material.angular.io/guide/material-2-theming#defining-a-theme) below.
...
Angular Material represents a theme as a Sass map that contains your color, typography, and density choices, as well as some base design system settings. See [Angular Material Typography](https://material.angular.io/guide/typography) for an in-depth guide to customizing typography. See [Customizing density](https://material.angular.io/guide/material-2-theming#customizing-density) below for details on adjusting component density.

Constructing the theme first requires defining your primary and accent palettes, with an optional warn palette. The m2-define-palette Sass function accepts a color palette, described in the [Palettes](https://material.angular.io/guide/material-2-theming#palettes) section above, as well as four optional hue numbers. These four hues represent, in order: the "default" hue, a "lighter" hue, a "darker" hue, and a "text" hue. Components use these hues to choose the most appropriate color for different parts of themselves.
...
The recommended approach is to rely on the theme mixins to lay down your base styles, and if needed use the single dimension mixins to override particular aspects for parts of your app (see the section on [Multiple themes in one file](https://material.angular.io/guide/material-2-theming#multiple-themes-in-one-file).)
...
These files include the CSS for every component in the library. To include only the CSS for a subset of components, you must use the Sass API detailed in [Defining a theme](https://material.angular.io/guide/material-2-theming#defining-a-theme) above. You can [reference the source code for these pre-built themes](https://github.com/angular/components/blob/main/src/material/core/theming/prebuilt) to see examples of complete theme definitions.
...
Using the Sass API described in [Defining a theme](https://material.angular.io/guide/material-2-theming#defining-a-theme), you can also define multiple themes by repeating the API calls multiple times. You can do this either in the same theme file or in separate theme files.
...
Angular Material's density customization is based on the [Material Design density guidelines](https://m2.material.io/design/layout/applying-density.html). This system defines a scale where zero represents the default density. You can decrement the number for more density and increment the number for less density.
...
The following example shows a typical theme definition and a "kids theme" that only applies when the ".kids-theme" CSS class is present. You can [see the theming guide for more guidance on defining multiple themes](https://material.angular.io/guide/material-2-theming#defining-multiple-themes).
...
It is possible to read typography properties from a theme for use in your own components. For more information about this see our section on [Theming your own components](https://material.angular.io/guide/material-2-theming#theming-your-components),
...
* M3 has a different API for setting the color variant of a component (see [using component color variants](https://material.angular.io/guide/material-2-theming#optional-add-backwards-compatibility-styles-for-color-variants) for more).
